...
// Abstract class as the injection token (NestJS DI swap point)
@Injectable()
...
export class ChaosKitchenService extends KitchenService {
    prepare(orderId: number, drink: string): Preparation {
        ...
        if (roll < 0.5) {
            ...
            console.log(`[Chaos] Injecting ${Math.round(delay)}ms latency for order #${orderId}`);
            const start = Date.now();
            while (Date.now() - start < delay) {
                // busy wait (simulates blocking latency in sync context)
            }
        }
